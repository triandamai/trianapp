<template>
    <a class="relative mx-auto block w-48 overflow-hidden rounded-lg bg-slate-200 shadow-xl shadow-slate-200 sm:w-64 sm:rounded-xl lg:w-auto lg:rounded-2xl"
        aria-label="Homepage" href="/"><img alt="" sizes="(min-width: 1024px) 20rem, (min-width: 640px) 16rem, 12rem"
            src="https://avatars.githubusercontent.com/u/41132846?v=4" decoding="async" data-nimg="future"
            class="w-full" width="960" height="960">
        <div class="absolute inset-0 rounded-lg ring-1 ring-inset ring-black/10 sm:rounded-xl lg:rounded-2xl">
        </div>
    </a>
</template>